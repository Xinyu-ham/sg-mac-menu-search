...
class Index:
    ...
    @classmethod
    def create_index_from_properties(cls, es: Elasticsearch, name: str, properties: dict) -> 'Index':
        mapping = {
            'properties': properties
        }
        return cls(es, name, mapping, [])
    ...
    def save_records(self, df, batch_size : int| None=100):
        for batch in range(0, self.size, batch_size):
            sub_df = df.iloc[batch:batch+batch_size]
            actions = self.add_records_from_dataframe(self.name, sub_df)
    ...
    def search(self, search_field: str, query: str):
        embedded_query = self.embed_search(query)
        ...
        record = self.make_record_from_response(response['hits']['hits'][0])
        return record, score


    def embed_search(self, query: str) -> list[float]:
